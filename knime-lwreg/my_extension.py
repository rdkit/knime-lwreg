...
@knext.input_table(
    ...
    description="Input table containing molregnos (and optional conf_ids) to query.",
)
@knext.output_table(
    name="Query Results",
...
class LWRegQueryNode:
    ...
    # String input to specify the database path
    query_column_name = knext.StringParameter(
        label="Query Column",
        description="Specify the column containing SMILES strings for querying.",
    ...
    def execute(self, exec_context, input_table):
        ...
        smis = input_df[self.query_column_name].tolist()
        ids_list = []
        for i, smi in enumerate(smis):
            try:
                # Call the query function with the user-specified query input
                query_results = lwreg.query(smiles=smi)
                assert (
                    len(query_results) == 1
                ), f"Length of query results is {len(query_results)}"
                ...
                if query_results and isinstance(query_results[0], tuple):
                    ...
                    ids_list.append(query_results[0])
                else:
                    # Results are single values (Molregno only), add NaN for Conf_ID
                    ids_list.append([query_results[0], np.nan])

                exec_context.set_progress(i / len(smis), "Query completed")

            except Exception as e:
                LOGGER.error(f"Query failed: {e}")
                ids_list.append([np.nan, np.nan])

        ids_df = pd.DataFrame(ids_list, columns=["molregno", "conf_id"])
        # Add the "Query" column first
        ids_df.insert(0, "Query", smis)  # Insert the query input as the first column
        exec_context.set_progress(1.0, "Query completed successfully.")
        return knext.Table.from_pandas(ids_df)
